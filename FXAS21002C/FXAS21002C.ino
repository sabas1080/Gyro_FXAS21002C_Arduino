...
#define SA0 0
#if SA0
#define FXAS21002C_ADDRESS 0x21  // SA0 is high, 0x1C if low
#else
#define FXAS21002C_ADDRESS 0x20
#endif
...
// Set up for data display
  ...
  byte c = readByte(FXAS21002C_ADDRESS, WHO_AM_I);  // Read WHO_AM_I register
  display.clearDisplay();
  display.setCursor(0,0); display.print("FXAS21000");  
  ...
  {  
    ...
    calibrateFXAS21002C(gBias);
    initFXAS21002C();  // init the accelerometer if communication is OK
...
{  
  ...
  if(readByte(FXAS21002C_ADDRESS, DR_STATUS) & 0x08)  // When this bit set, all axes have new data
  {
  ...
  byte source = readByte(FXAS21002C_ADDRESS, RT_SRC);  // Read the interrupt source register
...
///////////////////////////////////////////////////////////////////////////////
// Useful functions to access the FXAS21002C gyroscope
///////////////////////////////////////////////////////////////////////////////
...
void readGyroData(int16_t * destination)
{
  ...
  readBytes(FXAS21002C_ADDRESS, OUT_X_MSB, 6, &rawData[0]);  // Read the six raw data registers into data array
...
int8_t readTempData()
{
  return (int8_t) readByte(FXAS21002C_ADDRESS, TEMP);  // Read the 8-bit 2's complement data register 
}

 
void calibrateFXAS21002C(float * gBias)
{
  ...
  readGyroData(temp);
  readByte(FXAS21002C_ADDRESS, STATUS);
  
  FXAS21002CStandby();  // Must be in standby to change registers

  writeByte(FXAS21002C_ADDRESS, CTRL_REG1, 0x08);   // select 50 Hz ODR
  fcount = 50;                                     // sample for 1 second
  writeByte(FXAS21002C_ADDRESS, CTRL_REG0, 0x03);   // select 200 deg/s full scale
  uint16_t gyrosensitivity = 41;                   // 40.96 LSB/deg/s

  FXAS21002CActive();  // Set to active to start collecting data
   
  uint8_t rawData[6];  // x/y/z FIFO accel data stored here
  for(ii = 0; ii < fcount; ii++)   // construct count sums for each axis
  {
  readBytes(FXAS21002C_ADDRESS, OUT_X_MSB, 6, &rawData[0]);  // Read the FIFO data registers into data array
  ...
  FXAS21002CReady();  // Set to ready
}
  
  
// Set up sensor software reset
void FXAS21002CReset() 
{
writeByte(FXAS21002C_ADDRESS, CTRL_REG1, 0x20); // set reset bit to 1 to assert software reset to zero at end of boot process
delay(100);
while(!(readByte(FXAS21002C_ADDRESS, INT_SRC_FLAG) & 0x08))  { // wait for boot end flag to be set
}
display.clearDisplay();
display.setCursor(0,0); display.print("FXAS21000");
...
void initFXAS21002C()
{
  ...
  writeByte(FXAS21002C_ADDRESS, CTRL_REG0, gyroFSR);      // write FSR

  // Setup the 3 data rate bits, 4:2
  if(gyroODR < 8) { // data rate can only be 0 to 7
  writeByte(FXAS21002C_ADDRESS, CTRL_REG1, gyroODR << 2); 
  }
  ...
  writeByte(FXAS21002C_ADDRESS, CTRL_REG2,  0x0E);
  ...
  FXAS21002CActive();  // Set to active to start reading
}


// Sets the FXAS21000 to standby mode.
// It must be in standby to change most register settings
void FXAS21002CStandby()
{
  byte c = readByte(FXAS21002C_ADDRESS, CTRL_REG1);
  writeByte(FXAS21002C_ADDRESS, CTRL_REG1, c & ~(0x03));  // Clear bits 0 and 1; standby mode
}

// Sets the FXAS21000 to active mode.
// Needs to be in this mode to output data
void FXAS21002CReady()
{
  byte c = readByte(FXAS21002C_ADDRESS, CTRL_REG1);
  writeByte(FXAS21002C_ADDRESS, CTRL_REG1, c & ~(0x03));  // Clear bits 0 and 1; standby mode
  writeByte(FXAS21002C_ADDRESS, CTRL_REG1, c |   0x01);   // Set bit 0 to 1, ready mode; no data acquisition yet
}

// Sets the FXAS21000 to active mode.
// Needs to be in this mode to output data
void FXAS21002CActive()
{
  byte c = readByte(FXAS21002C_ADDRESS, CTRL_REG1);
 writeByte(FXAS21002C_ADDRESS, CTRL_REG1, c & ~(0x03));  // Clear bits 0 and 1; standby mode
 writeByte(FXAS21002C_ADDRESS, CTRL_REG1, c |   0x02);   // Set bit 1 to 1, active mode; data acquisition enabled
}
